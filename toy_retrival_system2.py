...
def build_gradio_app(retriever: RetrievalSystem) -> gr.Blocks:
    ...
    now with anoptional "Example problem" input that triggers a Q2Q retrieval.
    We assume that the retriever has already built its FAISS index.
    """
    def retrieve_fn(query: str, example: str, top_k: int) -> str:
        """
        Retrieve two sets of top-k results:
          1) For the main prompt-based retrieval (if query is supplied).
          2) For the question-to-question similarity retrieval (if example is supplied).
        """
    ...
    with gr.Blocks() as demo:
        ...
        submit_btn.click(fn=retrieve_fn, 
                         inputs=[query_input, example_input, top_k_input], 
                         outputs=[output_box])
